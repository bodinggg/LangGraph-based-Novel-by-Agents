...
7. 角色关系：与其他角色的关系（父亲、母亲、兄弟、姐妹、恋人、朋友、敌人、导师、学生等），包括关系描述和体现该关系的事件
    ...
    {{
        ...
        "relationships": [
            {{
                "source": "角色A",
                "target": "角色B",
                "relationship_type": "关系类型（如：父子、兄弟、恋人、朋友、敌对、师生等）",
                "description": "关系描述",
                "events": ["体现该关系的事件1", "体现该关系的事件2"]
            }}
        ]
    }}
]
...
WRITER_PROMPT ="""你是专注于【{genre}】类型的专业小说章节撰写人。请严格依据以下信息，创作符合整体架构的完整章节:

## 核心依据
1. 大纲内容：小说标题【{outline_title}】, 类型【{genre}】, 主题【{outline_theme}】, 背景【{outline_setting}】, 情节概要【{outline_plot_summary}】, 角色【{character_list}】
2. 上下文：前文内容【{pre_summary}】:...{pre_context}, 后文内容【{post_summary}】
3. 章节锚点：严格遵循【{chapter_outline_title}】；关键事件【{chapter_outline_key_events}】；场景【{chapter_outline_setting}】；摘要【{chapter_outline_summary}】
4. 角色驱动：依据【{character}】塑造角色行为和对话

## StoryBible 上下文（跨章节一致性）
6. 角色弧线状态：
{character_arcs}
7. 进行中的情节线/伏笔：
{active_plot_threads}
...
## 创作要求
1. 总字数需达 {word_count} 字符以上，情节丰富且细节适当
2. 采用 {genre} 类型的典型风格
3. 根据当前章节号{current_chapter_idx}与总章节数{num_chapters}控制节奏
4. 保持剧情发展的适当节奏，推进情节与角色发展
5. 创作尾声自然，以情节结束，可通过具体内容体现伏笔
6. 与前章实体内容保持连贯性
...
{{
  "title": "章节标题（与大纲一致）",
  "content": "多段落完整内容，需覆盖上述所有写作要求，确保不少于{word_count}字符",
  "notes": "本章具体推进点以及后续可衔接的延伸方向（可选）"
}}
只输出JSON内容，不要添加其他解释或说明。"""

REFLECT_PROMPT = """你是专业小说编辑，需要对章节进行全面评估并提供结构化的反馈。

## 评估维度
1. 情节发展(plot)：情节是否完整，逻辑是否合理，是否符合大纲要求
2. 角色塑造(character)：角色行为是否符合设定，对话是否自然，性格是否一致
3. 写作风格(style)：描写是否生动，语言是否流畅，文笔是否优美
4. 对话质量(dialogue)：对话是否自然，是否推进情节，是否符合角色特点
5. 节奏控制(pacing)：章节节奏是否恰当，紧张感是否合适
6. 场景描写(description)：环境描写是否生动，细节是否丰富
7. 逻辑一致性(logic)：前后逻辑是否一致，是否有矛盾之处
...
请用```json和```包裹JSON内容，确保符合以下结构:
{
  ...
  "feedback_items": [
    {
      "category": "问题类别(plot/character/style/dialogue/pacing/description/logic)",
  ...
  "overall_feedback": "整体评价总结",
  "plot_score": 情节评分(1-10),
  "character_score": 角色评分(1-10),
  "style_score": 文笔评分(1-10),
  "pacing_score": 节奏评分(1-10)
}

注意：
- 如果章节质量良好无明显问题，feedback_items可以为空数组
- 优先级high表示严重影响阅读体验的问题，medium表示需要改进的问题，low表示可选的优化建议
- 只输出JSON内容，不要添加其他解释或说明
        """
