...
2. 内容密度：总字数需达 {word_count} 字符以上，情节丰富且细节适当。
3. 风格适配：采用 {genre} 类型的典型风格（如悬疑类用短句 + 环境烘托紧张感，爱情类侧重情绪细节刻画，科幻类注重技术场景具象化）。
4. 节奏控制：根据当前章节号{current_chapter_idx}与总的章节数目{num_chapters}控制节奏，需符合 {genre} 类型的节奏要求。
5. 剧情发展：保持适当节奏，推进情节与角色发展。
6. 创作技巧：创作技巧需掌握，如对话技巧、角色塑造技巧、情节组织技巧、情节推进技巧、语言技巧等。
7. 创作尾声：自然，以情节结束，不表达对后续内容的展望，但是可以通过具体内容体现伏笔。
8. 前章实体：如果前章实体内容存在，那么需与前文内容中提到的角色、组织、地点、事件等实体保持连贯性。
