...
## 功能特点
- **全流程自动化**：LangGraph 状态流转 + **意图解析**→规划→执行→评估的端到端链路，最后生成完整小说
- **设计架构**：分层规划机制 + 动态重试策略 + 质量反馈闭环，融合 Plan-and-Execute 与 ReAct 的双模式驱动
- **结构化生成**：Pydantic schema 定义 + JSON Schema 校验 + 领域专属字段约束，正则预处理模型响应后解析JSON，并为各Agent输出定义强类型Schema。
- **多 Agent 协作**：基于状态订阅的去中心化协同， 采用全局不可变状态 + Agent 间事件驱动通信。
- **迭代优化机制**：多层级验证与定向修正体系。
- **可配置参数**：创作策略的精细化参数化管控，由YAML配置。
- **错误反馈**：可溯源的智能归因与解决方案提示。
- **过程可视化**：生成的每部分内容都支持持久化本地，可以实时查看生成的大纲/角色档案/章节信息，前端有更优雅的界面
- **模型支持**：本地 / API 双模式兼容
- **Prompt工程**：场景化的动态模板体系，分层Prompt模板 + 变量注入 + Token优化
